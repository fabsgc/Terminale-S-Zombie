	...
	$('a').click(function(){
		if($(this).attr('rel')=='ConcentLink'){
			...
			if(params.CSSstyles==true){
				$('.'+params.AscensorName+'NavigationButton').css('background','#c0392b');
				$('.'+params.AscensorName+'NavigationButton:eq('+params.WindowsOn+')').css('background','#681c14');
			}
...
//**********************************************************************************************
	...
	if(params.Navig==true){
		...
		$(node).children('div').each(function(index) {
			$('#'+params.AscensorName+'Navigation').append('<dt class="'+params.AscensorName+'NavigationButton" ><a href="#" title="Link for Content"  id="'+params.AscensorName+'NavigationButton'+index+'"></a></dt>').find('dt:eq('+params.WindowsOn+')').css('background','#c0392b');
	...
	if(params.CSSstyles==true){
		...
		$('#'+params.AscensorName+'Navigation dt').css({'z-index': '20000','position': 'fixed','width':'33px','height':'33px','background':'#c0392b'});
		$('#'+params.AscensorName+'Navigation dt:eq('+params.WindowsOn+')').css({'background': '#681c14'});
		...
		if(params.ChocolateAscensor==true){
			...
			$('#'+params.AscensorName+'Navigation').find('dt').each(function(index) {
				...
				var positionY= MapHeight*3.1;
				
				var positionX= MapWidth*35;
				
				
				$(this).css('bottom', function() {return (CoordNavY*40+positionY); });
				$(this).css('right', function() {return (1*CoordNavX*40+positionX);});		

				
			});
		}else{
		$('#'+params.AscensorName+'Navigation').find('dt').each(function(index) {
			$(this).css('bottom', function() {return (index*20); });		
	...
	$('.'+params.AscensorName+'NavigationButton').click(function(){
    	...
    	$.scrollTo(($('.'+params.AscensorName+':eq('+AscensorFinal+')')), 500, {axis: params.NavigationDirection,onAfter:function(){
    		if(params.ReturnURL==true){
    	...
    	if(params.CSSstyles==true){
			$('.'+params.AscensorName+'NavigationButton').css('background','#c0392b');
			$(this).css('background','#681c14');
		}
    });
	
		$('.'+params.AscensorName+'NavigationButton').hover(function(){
			$(this).css('background-color', '#681c14');
			$('.'+params.AscensorName+'NavigationButton:eq('+params.WindowsOn+')').css('background-color', '#681c14');
			}, function(){
...
//**********************************************************************************************
	...
	if(params.PrevNext==true){
		$(node).append('<a id="'+params.AscensorName+'Prev"  href="?">précédent</a><a id="'+params.AscensorName+'Next" href="?">suivant</a>');
	}
	    ...
	    if(params.CSSstyles==true){
	    	$('#'+params.AscensorName+'Prev').css({'position':'fixed', 'z-index':'20000', 'bottom':'28px', 'left':'32px', 'background-color':'#c0392b', 'text-align':'center', 'width':'126px', 'color':'white', 'padding':'5px 0px 5px 0px'});
	    }
	    		
	   
		
		if(params.CSSstyles==true){
			 $('#'+params.AscensorName+'Next').css({'position':'fixed', 'z-index':'20000', 'bottom':'28px', 'left':'180px', 'background-color':'#c0392b', 'text-align':'center', 'width':'126px', 'color':'white', 'padding':'5px 0px 5px 0px'});
		}
